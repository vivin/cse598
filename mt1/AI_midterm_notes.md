...
Here `f(n) = h(n)`.
 ...
 - **Optimality**: It is not optimal, and to see why, consider a path A-B-C and A-C. The heuristic cost from A-B is 50, the cost from B-C is 90, and the cost from A-C is 100. Greedy best-first search will choose B for expansion because 50 is lesser than 100. However, the path from A to C via B is 40 more than the path from A to C directly. Hence it is not optimal. 
